...
public class MongoConnectionTest {
    ...
    public void getUrlByPeriodTest() {
        Log log = logsForTest.get(0);
        connTestObject.create(log);
        Assert.assertEquals(true, connTestObject.getURLbyPeriod(new Timestamp(1000000), new Timestamp(1000000000)).get(0).contains(log.getURL()));
    }

    @Test
    public void getUrlTime() {
        Log log = logsForTest.get(0);
        connTestObject.create(log);
        Assert.assertEquals(true,connTestObject.getMapReducedUrlTime().get(0).contains(String.valueOf(log.getTimeSpent())));

    }
    @Test
    public void getUrlCountTest() {
        Log log = logsForTest.get(0);
        connTestObject.create(log);
        Assert.assertEquals(true,connTestObject.getMapReducedUrlCount().get(0).contains("1"));
    ...
    public void getUrlCountByPeriodTest() {
        Log log = logsForTest.get(0);
        connTestObject.create(log);
        Assert.assertEquals(true,connTestObject.getMapReducedUrlCountByPeriod(new Timestamp(1000000),new Timestamp(100000000)).get(0).contains(String.valueOf(log.getURL())));
    }
    @Test
    public void getIpMapTest(){
        Log log = logsForTest.get(0);
        connTestObject.create(log);
        Assert.assertEquals(true,connTestObject.getMapReducedIpCountAndTime().get(0).contains(String.valueOf(log.getIP())));
        Assert.assertEquals(true,connTestObject.getMapReducedIpCountAndTime().get(0).contains(String.valueOf(log.getTimeSpent())));
    }
    @Test
